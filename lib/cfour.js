  ...
  $.fn.cfour = function() {        
    return this.each(function() {
      ...
      var self = {   
        ...
        animate: function(callback, delay, iterations, column) {
          var counter = 0;
          var interval = setInterval(function() {
            counter++;
            console.log(counter);
            if (callback) { callback.call(self, counter, column); }
            if (counter >= iterations) { clearInterval(interval); }
          }, delay);
        },

        start: function(counter, column) {
        ...
        move: function(colour, column) {
          ...
          this.animate(this.start, 10, (6 - countersInColumn) * 4, column);
        }
      };
      this.cfour = self;
